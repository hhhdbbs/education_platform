...
<script>
  ...
  export default {
    ...
    this.$axios.post('')
      .then(res=>{
          if(res.status==200){
            console.log(res);
            for(var k=0;k<res.data.teachers.length;k++){
              var kk=res.data.teachers[k]
              var kkk={
                 id: kk.teacher_id,
        label: kk.name,
        show_name:true,
        ...
        children: []
          ...
          ///复制
   for(var i=0;i<kk.courses.length;i++){
              var dd=kk.courses[i]
              var course={id:dd.id,
              label:dd.course,
               show_name:true,
                ...
                course.children.push(
  {
        ...
        show_del:true,
              ...
              kkk.children.push(course)
            }
          ////粘贴
          this.data.data.push(kkk)
            }
            }
        })
    ...
    methods: {
      ...
      seeAll(node, data){ 
        if(node.level == 1){//查看老师
          console.log('edit course'+data.id)
        //  this.$router.push({name: 'user', params: {userId: data.id}})
        //希望可以去别人主页
        }
        else if(node.level ==2){//查看课程
          console.log('edit course'+data.id)
          this.$router.push({name: 'course_info', params: {courseId: data.id}})
        }
        else if(node.level == 3&&!(data.is_exer==true)){//查看课时
